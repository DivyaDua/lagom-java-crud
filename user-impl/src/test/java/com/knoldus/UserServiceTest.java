...
public class UserServiceTest {
    ...
    @Test
    public void shouldPublishToUsersTopic() {
        User user = User.builder().id("1").name("testUser").age(10).build();
        userService.createUser().invoke(user).toCompletableFuture();
        
        Source<User, ?> source = userService.usersTopic().subscribe().atMostOnceSource();
        
        TestSubscriber.Probe<User> probe =
                source.runWith(
                        TestSink.probe(testServer.system()), testServer.materializer()
                );
        
        User actualUser = probe.request(1).expectNext();
        assertEquals("Unable to subscribe to the users", user, actualUser);
    }
}
